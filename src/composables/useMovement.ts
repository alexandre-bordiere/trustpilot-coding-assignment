...
export default function useMovement(maze: Maze): UseMovement {
  ...
  const canMoveToCellAbove = (position: number) =>
    !isFirstRow(position) && !maze.structure[position].includes('north')
  const canMoveToCellBelow = (position: number) =>
    !isLastRow(position) &&
    !maze.structure[getAdjacentCellPosition(position, 'south')].includes(
      'north'
    )
  const canMoveToNextCell = (position: number) =>
    !isLastColumn(position) &&
    !maze.structure[getAdjacentCellPosition(position, 'east')].includes('west')
  ...
  const getShortestPath = (
    ...
    const isFirstMovement = oldPosition === undefined

    const movements = getPossibleMovements(currentPosition).filter(
    ...
    for (const { direction, newPosition } of movements) {
      if (isFirstMovement || movements.length > 1) {
        const content =
          !isFirstMovement && typeof paths[baseIndex] !== 'undefined'
            ? paths[baseIndex]
            : []
    ...
    return paths
      .filter(movements =>
        movements.some(movement => movement.newPosition === maze.positions.end)
      )

...
export default function useCell(maze: Maze): UseCell {
  const cellsCount = computed(
    () => maze.dimensions.height * maze.dimensions.width
  )
  ...
  const isFirstColumn = (position: number) =>
    position % maze.dimensions.width === 0
  const isLastColumn = (position: number) =>
    (position + 1) % maze.dimensions.width === 0
  const isFirstRow = (position: number) => position < maze.dimensions.width
  const isLastRow = (position: number) =>
    cellsCount.value - position <= maze.dimensions.width
